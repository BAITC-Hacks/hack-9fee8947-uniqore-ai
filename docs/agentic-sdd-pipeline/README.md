...
План процесса разработки через Codex и Claude Code для HackAlem AI. Ветка: `agentic-sdd-pipeline`. Дата: 2026-09-23. Статус: review завершено, замечания внесены; процесс ожидает пользовательского принятия. Автоматический orchestrator и продуктовый пилот ещё не реализованы.
...
Начать: [первый запуск в три шага](quickstart.md). Правила: [spec](process-spec.md), [контракты](contracts.md). Выполнение: [prompts](prompts.md), [бюджет и этапы](implementation-plan.md). Проверка: [acceptance](acceptance.md), [отчёт review](review.md).
...
Только Coordinator пишет `run.json` и статусы задач в отдельном control worktree/branch конкретного run. Spec, tasks, review и evidence хранятся там; их commit ссылается на immutable SHA кода. Control branch не сливается в code candidate: запись результата проверки не меняет проверенный код. Workers получают packet из control worktree и создают код в своих worktrees. Reviewer не меняет код при review, чтобы не проверять собственное исправление.

По умолчанию пользователь ведёт Coordinator/Integrator, Роман — продуктовые решения; это назначение фиксируется в inputs.json перед запуском. Первый пилот выполняется на одном host: control, worker A и integration используют три отдельных worktree; второй worker добавляет четвёртый. Структура и порядок создания — в [quickstart.md](quickstart.md). Если участники работают на разных компьютерах, используется описанная в [contracts.md](contracts.md) передача Git bundle либо явно разрешённый remote: один SHA без Git objects не является переданной работой.
...
Для автоматической проверки нужны фактический запуск и exit code; для ручной — шаги, наблюдение, проверяющий и target SHA. Нет DONE только на основании фразы агента «готово». Изменение реализации допустимо внутри поведения spec; изменение поведения сначала проходит change request к источнику истины.
