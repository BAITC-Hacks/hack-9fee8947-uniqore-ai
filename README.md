...
![Рабочий интерфейс: постоянные фильтры, очередь, граф и досье](docs/finance/screenshots/ux-tyuin-workbench-1280.png)
...
Активация окружения и изменение `ExecutionPolicy` не нужны. Команды выше фиксируют Python 3.12 для повторения SDD-проверки. Если установлен Python 3.14, используйте `py -3.14 -m venv .venv`; он также поддерживается. Без launcher укажите полный путь к выбранному Python: [выбор интерпретатора, проверка и ошибки запуска](docs/local-launch.md).
...
Проверка установки выполнена в отдельном виртуальном окружении и копии проекта: macOS arm64 (Python 3.12.1) и Windows 11 x64 (Python 3.14.5, PowerShell, чистый `git clone` с `core.autocrlf=true`). Дополнительно проверен Docker-контейнер Linux amd64: сборка, запуск, API, CSV и 29 тестов; [результаты](docs/deployment/brev.md#проверка-конфигурации). Запуск Python напрямую в Linux в этой итерации не проверялся.

Расширенная приёмка Windows с Python 3.12 ведётся отдельно по [спецификации запуска](docs/specs/local-launch.md). Preflight проверяет окружение, данные и файлы интерфейса, но сам анализ не запускает.
