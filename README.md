...
**Tyuin помогает AML-аналитику выбрать, кого проверить первым и почему.** Из трёх Parquet приложение рассчитывает роли, сообщества и top-20, связывает очередь с направленным графом и досье. Можно использовать предоставленные 2 248 клиентов или загрузить собственный набор в поддерживаемом формате. Роль раскрывается до признаков и порогов, связь — до исходных операций. Основной сценарий работает локально, без аккаунтов команды, GPU и внешнего AI-сервиса.
...
Открыть **[http://127.0.0.1:8765](http://127.0.0.1:8765)**. Остановить сервер — `Ctrl+C`. Если порт занят, добавить `--port 8766`. Сервер по умолчанию слушает только локальный адрес.
...
**[Открыть онлайн-демо Tyuin](https://hackalemai.uniqore.dev)** — приложение на CPU-инстансе `tyuin` в NVIDIA Brev, прямой HTTPS через Caddy с сертификатом Let's Encrypt. Вход в Brev не нужен. Опубликованы демонстрационные данные кейса, API и три CSV. Код приложения — commit `3c6b1cd`, для которого ранее прошли 29 тестов на VM. На собственном домене проверены HTTPS, перенаправление с HTTP, поиск клиента, API, объяснения, совпадение SHA-256 трёх CSV и работа после перезапуска Caddy. [Настройка, ограничения и результаты проверки](docs/deployment/brev.md#собственный-домен-и-https).

Онлайн-демо использует алгоритм 1.0.0 и снимок `ad6239d9995448b0`; новые сценарии интерфейса, загрузка собственных данных и алгоритм 1.2.0 из текущей `main` ещё не развёрнуты. Инструкции локального запуска и сценарии ниже относятся к текущему коду репозитория.
...
| macOS arm64, Python 3.12.1, Node 25.8.1, загрузка данных | 127 Python-тестов, 39 frontend-тестов, build, импорт/ошибки/переключение/AI-контекст в браузере; свежий запуск CLI/API/CSV/assets — 4,641 с | [Проверка загрузки собственных данных](docs/finance/custom-upload-validation.md) |
...
Исходный кейс: один банк, июль 2026, исходящий обход на четыре колена и суммы от 5 000 KZT. Собственный набор должен соблюдать ту же структуру обхода и порог сумм, но может иметь другой период. Внешние входы, остатки и продолжение границы неизвестны. Нет эталонных ролей, поэтому accuracy, recall и доказанная эффективность расследования не заявляются. Сообщества не равны преступным группам; все назначения — гипотезы.
