...
Предыдущая проверка установки выполнена в отдельном виртуальном окружении и копии проекта: macOS arm64 (Python 3.12.1) и Windows 11 x64 (Python 3.14.5, PowerShell, чистый `git clone` с `core.autocrlf=true`). Для предыдущей версии отдельно проверен Docker-контейнер Linux amd64: сборка, запуск, API, CSV и 29 тестов; [результаты и проверенный commit](docs/deployment/brev.md#проверка-конфигурации). Эти проверки не являются повторным прогоном текущего UX-сценария; новая итерация проверена на macOS.
